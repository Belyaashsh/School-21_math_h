...
int s21_abs(int x) {  // работает (руч)
  if (x < 0) {
    x *= (-1);
  }
  return x;
...
long double s21_ceil(double x) {  // работает (руч)
...
long double s21_fabs(double x) {  // работает (руч)
  if (x < 0) {
    x *= -1;
  }
  return x;
...
long double s21_floor(double x) {  // работает (руч)
...
long double s21_exp(double x) {  // работает (руч)
  long double ans = 1;
  long double new_x = x;
  bool flag = false;
  bool znak = false;
...
long double s21_log(
    double x) {  // работает, но не обработана ошибка при 0 и отриц. значенитях
  long double res = 0;
...
long double s21_sqrt(double x) {  // работает (руч)
  long double res = 0;
  if (x == -0) {
    return -0;
  }
